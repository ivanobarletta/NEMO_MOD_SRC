...
CONTAINS
 ...
 9202    FORMAT(' ',i3,' *  ',20(i3,'  x',i3,'   *   '))
      ...
      IF ( jpnij > 1 ) THEN
         DO ji = 2, jpnij
            DO jn = 1,ji-1
               RB(ji) = RB(ji-1) + ( nleit(jn) - nldit(jn) + 1 ) * ( nlejt(jn) - nldjt(jn) + 1 )
            END DO
         END DO 
      END IF 
      ...
      IF ( someone_east(narea) == .TRUE. ) THEN
         DO jj = nldjt(narea), nlejt(narea)
            col_shift_east(jj) = (nlejt(narea)-jj) * nx_self(narea) + (jj-nldjt(narea)) * nx_east(narea) + 1   
         END DO                          
      END IF

      IF ( someone_west(narea) == .TRUE. ) THEN
         DO jj = nldjt(narea), nlejt(narea)
            col_shift_west(jj) = (jj-nldjt(narea)) * nx_self(narea) + (nlejt(narea)-jj) * nx_west(narea) + 1   
         END DO                          
      END IF

      IF ( someone_south(narea) == .TRUE. ) THEN
         DO ji = nldit(narea), nleit(narea) 
            col_shift_south(ji) = (ji - nldit(narea)) + (RB(narea) - RB(narea-jpni+1)) + (nleit(narea) - ji) + 1
         END DO
      END IF

      IF ( someone_north(narea) == .TRUE. ) THEN
         DO ji = nldit(narea), nleit(narea)
            col_shift_north(ji) = (nleit(narea) - ji) + (RB(narea+jpni) - RB(narea+1)) + (ji-nldit(narea)) + 1 
         END DO
      END IF
